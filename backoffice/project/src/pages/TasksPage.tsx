...
export function TasksPage() {
  ...
  const filtered = useMemo(() => {
    const rows = tasks.filter((t) => {
      ...
      if (!inRange(t.task_date.slice(0, 10), dateRange)) return false;
      return true;
    });
  ...
  }, [tasks, activeTab, projectFilter, employeeFilter, priorityFilter, departmentFilter, dateRange, employeeDeptMap]);

  const hasFilters = !isDefaultRange(dateRange) || projectFilter !== 'all' || departmentFilter !== 'all' || employeeFilter !== 'all' || priorityFilter !== 'all';
