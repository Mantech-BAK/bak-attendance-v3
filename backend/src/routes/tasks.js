...
router.post('/', async (req, res, next) => {
  try {
    const { emp_id, project_code, priority, description, location, source, created_by } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO tasks (emp_id, task_date, project_code, priority, description, location, source, created_by)
       VALUES ($1, CURRENT_DATE, $2, $3, $4, $5, $6, $7)
       RETURNING id, emp_id, task_date, project_code, priority, description, location, status, source, created_by, created_at`,
      [emp_id, project_code, priority || null, description, location || null, source, created_by]
    );
